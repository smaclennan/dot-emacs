...
;; SAM Change: Always ask for the server unless passed as an arg.
;;             To specify the port, use a : and the port number.
(defun gopher (server &optional port)
  "Start a gopher session.  Interactive, prompt for a gopher server."
  (interactive "sServer: ")
  (unless  port
    (if (string-match ":\\([0-9]+\\)$" server)
	(progn
	  (setq port (match-string 1 server))
	  (setq server (substring server 0 (match-beginning 0))))
      (setq port 70)))
  (when (stringp port) (setq port (string-to-number port)))

  (gopher-set-object-host gopher-root-node server)
...
(defun gopher-get-tmp-buf nil
  ...
  (or (bufferp gopher-tmp-buf)
	...
	(buffer-disable-undo gopher-tmp-buf)))
  gopher-tmp-buf)
