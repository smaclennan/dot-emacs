...
(eval-when-compile (require 'sam-common) (require 'tmp-buffer))

(defvar cpuinfo-bufname "*cpu-info*"
  "* Buffer name for cpuinfo-get.")

(my-feature-cond
  (xemacs (defalias 'my-set-buffer-modified 'set-buffer-modified-p))
  (emacs (defun my-set-buffer-modified (flag &optional buffer)
	   (if buffer
	       (save-current-buffer
		 (set-buffer buffer)
...
If the buffer already exists, do nothing."
  (let ((buffer (get-buffer cpuinfo-bufname)))
    (if buffer
	(my-set-buffer-modified t buffer)
      (setq buffer (tmp-buffer-create cpuinfo-bufname t t))
...
(defun cpuinfo-put ()
  (let ((buffer (get-buffer cpuinfo-bufname)))
    (if buffer
	(my-set-buffer-modified nil buffer))))
...
`vendor' is a string, all others are numbers."
  ...
  (let (vendor family model step)
    ...
    (when (interactive-p)
      (message "Vendor %s Family %d Model %d Step %d"
