...
(defvar endif-if-re (concat endif-sol-re "if\\(n?def\\>\\|[ \t]+!?[ \t]*defined[ \t]*(\\)")
  "* Match an ifdef, or if defined or if !defined.")
...
(defun endif-goto (highlight)
  "Goto the endif that matches the current if or else line."
  (interactive "P")
  (let ((saved-point (point)) start (nesting 1))
    (beginning-of-line)
    (unless (or (looking-at endif-if-re)
		(looking-at endif-else-re))
      (goto-char saved-point)
      (error "Not on an if or else line"))
...
(defun endif-next-if (highlight)
  ...
  (when (re-search-forward endif-if-re)
    (beginning-of-line)
    (when (or highlight endif-highlight)
      (save-excursion
	(endif-goto t)))))
