...
(if (featurep 'xemacs)
  ...
  (load "hex-util" t)
  (eval-when-compile (load "hex-util" t))
  )

(defun my-sha1-binary (string) (sha1 string nil nil t))
...
(defun ocra-generate (question secret &optional suite)
  (let ((power 100000000) ;; default 8
    ...
    (setq secret (decode-hex-string secret))
    ...
    (setq hash (hmac secret msg 'my-sha1-binary sha-digest-length))
