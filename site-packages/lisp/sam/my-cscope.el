...
(mcs-feature-cond
 (emacs
  ;; I tried to use the compilation like XEmacs... but it only worked
  ...
  (defun mcs-lookup (cmd)
	...
	(erase-buffer)
	(insert cmd "\n\n")
	(unless (eq (call-process-shell-command cmd nil t) 0)
	  (error "cscope failed"))
	...
	(while (re-search-forward mcs-regexp nil t)
	  (setq count (1+ count))
	  (replace-match (concat (match-string 1) ":" (match-string 3) ":1" (match-string 2) (match-string 4))))
 ...
 `compilation-error-regexp-alist'.")
  ...
  (defun mcs-cscope-finished (buffer desc)
	...
	(when (eq (length list) 1)
	  (setq list (car list))
	  ;; SAM can XEmacs use first-error() ?
