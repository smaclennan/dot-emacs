...
(defun mcs-maybe-goto ()
  "Called from the compilation finished scripts."
  (let (list)
    (goto-char (point-min))
  ...
  (emacs
   ...
   (defun mcs-cscope-finished (buffer desc)
     (when (equal (buffer-name buffer) "*cscope*")
       ...
       (mcs-maybe-goto)))
   ;; This doesn't hurt since it only does anything on cscope buffers
...
`my-cscope-prog' `my-cscope-args' -L -<TYPE><SYM>."
  ...
  (let ((default-directory (mcs-dir default-directory))
