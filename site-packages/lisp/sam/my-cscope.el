...
(defvar mcs-base nil "Local variable")
...
(defun mcs-lookup (sym)
  ...
  (let ((cmd (concat my-cscope-prog " " my-cscope-args " -L -" sym))
	...
	(compilation-parse-errors-filename-function 'mcs-filename-func)
	list this)
...
(defun mcs-goto (this)
  (let ((filename (nth 0 this))
    ...
    (find-file filename)
    ;; Ignore goto-line is for interactive use only warning
    (with-no-warnings (goto-line line))))

(defun mcs-filename-func (filename)
  (concat mcs-base filename))
...
(defun my-cscope (type sym)
  ...
  (let ((dir (mcs-dir default-directory)))
    ...
    (let ((default-directory dir)
	  list)
      (setq list (mcs-lookup (concat (number-to-string type) sym)))
      (push-tag-mark)
      (message "list %S" list);; SAM DBG
      (if (eq (length list) 1)
	  (mcs-goto (car list))
