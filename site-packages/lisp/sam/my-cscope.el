...
(defun my-cscope (type &optional sym)
  "Call cscope on SYM of type TYPE. The default TYPE is 1; \"find this
function definition\". Set a prefix arg for TYPE. TYPE must be 0 to 9
inclusive (see man scscope for types).

SYM defaults to the current word at the point.

If there is only one result, then `my-cscope' will just goto the
symbol. If there are multiple results, then a compilation type buffer
...
`my-cscope-prog' `my-cscope-args' -L -<TYPE><SYM>."
  ...
  (unless sym
    (let ((word (current-word)))
      (setq sym (read-string (concat "Sym [" word "]: ") nil nil word))))
