...
(defcustom my-cscope-use-native t
  "* If non-nil use the native Emacs interface if possible.
This is either faster or slower... depends on the machine." :type 'bool)
...
(defun native-cscope-to-buffer (sym tagname)
  (let ((full-match (concat "^\\(." sym "\\)\t\\([0-9]+\\)"))
	(match (concat sym "\t"))
	(case-fold-search nil) ;; C is case sensitive
	(count 0)
	mark str line file list)
    (with-current-buffer (find-file-noselect tagname)
      (goto-char (point-min))
      ;; Wayyyy too expensive to do an re-search here
      (while (search-forward match nil t)
	(setq mark (point))
	(beginning-of-line)
	(when (looking-at full-match)
	  (setq str  (match-string 1)
		line (match-string 2))
	  ;; This is not expensive
	  (re-search-backward "^@\\(.*\\)")
	  (setq file (match-string 1))
	  (setq list (append list (list (list file line str)))))
	(goto-char mark) ;; make sure we move forward
	))

    ;; Back in original, empty, buffer
    (insert "-*- default-directory: \"" default-directory "\" -*-\n\n")
    (dolist (entry list)
      (setq count (1+ count))
...
`my-cscope-prog' `my-cscope-args' -L -<TYPE><SYM>."
  ...
  (let ((count 0) tagname)
    (with-current-buffer (get-buffer-create "*cscope*")
      ...
      (if (and my-cscope-use-native (eq type 1) (file-readable-p tagname))
	  (setq count (native-cscope-to-buffer sym tagname))
