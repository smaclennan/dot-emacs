...
;; (git-fname "/home/work/git-monza/software/kernel/core/pikacore.c")


(defun git-cat (&optional rev)
  "Perform an git show on the current buffer into a temporary buffer.
...
When called interactively, the name of the temporary buffer will be displayed."
  ...
  (let* ((full-path (buffer-file-name))
	 ...
	 (catname (concat ":" git-fname)))
    (when rev (setq catname (concat catname "@" rev)))
    (empty-buffer buf)
    (call-process "git" nil buf nil "show" catname)
    (set-buffer-modified-p nil buf)
    (when (interactive-p) (message "git cat to buffer %s" (buffer-name buf)))
    buf))


;;;###autoload
(defun git-diff (rev)
  "Perform an git diff against the current buffer using ediff.
