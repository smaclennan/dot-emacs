...
(defun my-kernel-vers ()
  "Splits `my-kernel-vers' up into a list of (MAJOR MINOR PATCH RC
EXTRA). All are numbers except EXTRA; which is a possibly empty
string."
  (if (string-match "\\([0-9]+\\).\\([0-9]+\\).\\([0-9]+\\)\\(.*\\)" my-kernel-vers)
      (let ((vers (list (string-to-number (match-string 1 my-kernel-vers))
...
(defun my-kernel>= (major minor &optional patch)
  "Return true if the `my-kernel-vers' is >= to the given MAJOR, MINOR,
   and PATCH numbers.
The MAJOR and MINOR version numbers are required, but the PATCH
...
The first element can also be svn://<url>.
