...
;; Copyright (C) 2002-2004 Sean MacLennan
;; $Revision: 1.17 $ $Date: 2004/11/24 03:29:30 $
;; XEmacs
...
(defun smerge-post-process (flags)
  (let (match extent file start)
    ...
    ;; Only in 1
    (setq match (format "^Only in %s:? *\\(.*\\)$" (regexp-quote smerge-dir1)))
    (goto-char (point-min))
    ...
    ;; Only in 2
    (setq match (format "^Only in %s:? *\\(.*\\)$" (regexp-quote smerge-dir2)))
    (goto-char (point-min))
    (while (re-search-forward match nil t)
      (setq file (match-string 1))
      (setq extent
    ...
    ;; Both
    (setq match (format "^Files %s\\(.+\\) and %s.+ differ$"
