...
(define-minor-mode caps-mode "Caps Mode" nil " Cm" cm-keymap)

(defun cm-init()
  (unless cm-keymap
    (setq cm-keymap (make-sparse-keymap "Caps Mode"))
    (define-key cm-keymap "." 'cm-period)
    (loop for ch from ?a to ?z do
      (define-key cm-keymap ch 'cm-alpha))))

(add-hook 'caps-mode-on-hook 'cm-init)
...
(defun cm-alpha (arg)
  ...
  (if (or cm-up-next-char (bobp))
      (progn
	(insert (char-upcase last-command-char))
