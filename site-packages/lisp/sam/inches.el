...
;;;###autoload
(defun lbs2kg (pounds)
  "Convert POUNDS to kgs. If you give two numbers it is lbs and oz."
  (interactive "slbs: ")
  (let (lbs kg)
    (setq lbs (string-to-number pounds))
    (when (string-match "[0-9]+[[:blank:]]+\\([0-9]+\\)" pounds)
      (setq lbs (+ lbs (/ (float (string-to-number (match-string 1 pounds))) 16))))
    (setq kg (/ lbs 2.20462))
    (message "%.2f kg" kg)
    kg))
