...
(defvar ogrok-smart t
  "*Try to be smart about matching rather than precise.")
...
(my-feature-cond
  ...
  (xemacs
   (defun ogrok-finish ()
     "Write out the buffer to a temp file and pass that to
`compile-internal'."
     (let* ((tmpfile (make-temp-file "ogrok"))
	    (cmd (concat "cat " tmpfile)))
       (write-file tmpfile)
       (setq compilation-finish-function 'ogrok-finished)
       (compile-internal cmd "No more entries." "ogrok")
       ))

   (defun ogrok-finished (buffer desc)
     "Called when `compile' is done. Delete the temp file."
     (when (equal (buffer-name buffer) "*ogrok*")
       (goto-char (point-min))
       (when (re-search-forward "^cat \\(.+\\)")
