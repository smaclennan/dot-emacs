...
(defun lxr-defined-at-point ()
  ...
  (let (ident)
    (if (region-exists-p)
	(setq ident (buffer-substring (region-beginning) (region-end)))
      (setq ident (current-word)))
  (lxr-defined ident)))
...
(defun lxr-defined-sentinel (proc str)
  (save-excursion
	...
	(push (list file line) lxr-defined-alist))
      (message "Found %d files." (list-length lxr-defined-alist)))
...
(unless running-xemacs (require 'extent))
(require 'http)
(require 'h2t)
(require 'etags)
(provide 'lxr)
