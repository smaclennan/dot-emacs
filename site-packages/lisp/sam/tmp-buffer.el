...
The function pointed to by `tbc-func' is used to create the buffer name."
  (let* ((bufname (tmp-buffer-name name reuse))
    ...
    ;; Start the timer
    (unless tbc-timer
      (setq tbc-timer (make-itimer))
      ;; (set-itimer-name tbc-timer "tbc cleanup") ;; SAM Not 21.4
      (set-itimer-is-idle tbc-timer t)
      (set-itimer-function tbc-timer 'tbc-check-buffers))

    (unless (itimer-restart tbc-timer)
      (set-itimer-value tbc-timer tbc-interval)
      (set-itimer-restart tbc-timer tbc-interval)
      (unless (itimer-live-p tbc-timer)
	(activate-itimer tbc-timer)))
...
(defun tbc-check-buffers ()
  ...
  (if tbc-list
	...
	(set-itimer-restart tbc-timer tbc-interval))
    ;; Disable
    (set-itimer-restart tbc-timer nil))
  )
