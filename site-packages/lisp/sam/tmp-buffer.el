...
(defun tmp-buffer-get (object)
  (let (found)
    (cond
     ((bufferp object)
      (dolist (tbc tbc-list)
	(when (eq object (car tbc))
	  (setq found tbc))))
     ((stringp object)
      (dolist (tbc tbc-list)
	(when (equal object (buffer-name (car tbc)))
	  (setq found tbc)))))
    found))

;;;###autoload
(defun tmp-buffer-p (object)
  "Returns the buffer if object was created by `tmp-buffer-create'.
Object can be a buffer name or buffer.
If arg is non-nil, it must match the arg in the tmp buffer."
  (let ((found (tmp-buffer-get object)))
    (when found (setq found (car found)))
    found))

;;;###autoload
(defun tmp-buffer-kill (object)
  "Kill oject if it was created by `tmp-buffer-create'.
Object can be a buffer name or buffer."
  (let ((found (tmp-buffer-get object)))
    (when found
      ;; Don't actually kill the buffer... mark it to expire aggressively
      (setcar (cdr found) 10))
    found))
