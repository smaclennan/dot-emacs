...
work."
  ...
  (let (args)
	...
	(setq isystem (shell-command-to-string (concat cc " -print-file-name=include")))
    ...
    (setq args (concat args "-I" my-kernel-dir "/arch/" my-sparse-linux-arch "/include "))

    ;; Add the generic includes
    (setq args (concat args "-I" my-kernel-dir "/include "))

    ;; Add the configs generated from .config
    (setq args (concat args "-include " my-kernel-dir "/include/generated/autoconf.h "))
...
(defvar my-checkpatch-prog "checkpatch"
  "* The checkpatch program.")

(defvar my-checkpatch-args "--emacs --file"
  "* Args to pass to checkpatch.")
...
compilation buffer so that `next-error' will work."
  ...
  (let ((cmd (format "%s %s %s" my-checkpatch-prog my-checkpatch-args (buffer-file-name))))
    (my-feature-cond
     (emacs (compilation-start cmd))
     (xemacs (compile-internal cmd "No more errors")))))
