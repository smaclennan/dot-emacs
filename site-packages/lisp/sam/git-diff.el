...
(eval-when-compile (require 'cl) (require 'tmp-buffer) (require 'ediff))

(eval-when-compile
  (if (fboundp 'called-interactively-p)
      (defmacro is-interactive () '(called-interactively-p 'interactive))
    (defalias 'is-interactive 'interactive-p)))
...
When called interactively, the name of the temporary buffer will be displayed."
  ...
  (let* ((full-path (buffer-file-name))
	 ...
	 (buf (tmp-buffer-create fname t nil 'git))
...
(defun git-ediff-cleanup ()
  ;; Because we ediff buffer against tmp-buffer, the tmp-buffer will
  ;; always be in ediff-buffer-B. `tmp-buffer-kill' will only kill
  ;; buffers created by `tmp-buffer-create'.
  (tmp-buffer-kill ediff-buffer-B))
...
With a prefix arg, ask for the revision. Otherwise defaults to HEAD."
  ...
  (when (and (is-interactive) rev)
    (setq rev (read-from-minibuffer "Revision: ")))
  (let ((buf (git-cat rev)))
    (add-hook 'ediff-quit-hook 'git-ediff-cleanup)
...
(defun git-status ()
  (interactive)
  (let ((buf (tmp-buffer-create "*git status*" t 'timeout)))
