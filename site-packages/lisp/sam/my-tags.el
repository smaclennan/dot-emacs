...
(defcustom my-tags-file nil
  "* If set, this is the directory `my-tags-update' should use.
This variable is buffer local.")
  ...
  (let ((buf (find-file-noselect tagfile t))
	...
	(revert-buffer nil t))
	  ...
	  (progn
	    ...
	    ;; Sanity check
	    (unless (eq (following-char) ?\f)
	      (undo)
	      (error "Not looking at Ctrl-L"))
	    (delete-char 2 t) ;; Delete the FF/NL pair
	    (basic-save-buffer)
	    )
	(message "%s not in tags file %s" fname tagfile)))))
...
(defun my-tags-update-helper (dir)
  ...
  (interactive "dDir: ")
  (setq my-tags-file (concat dir "TAGS"))
