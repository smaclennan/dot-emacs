...
(defun my-tag-config ()
  ...
  (let* ((fname (buffer-file-name))
	 ...
	 (git-dir (nth 1 (git-fname fname)))
	 (tags-fname tags-file-name))
    (and (not tags-fname)
	 (member 'my-tag-simple after-save-hook)
	 (setq tags-fname (concat dir "TAGS")))
	...
	(with-output-to-temp-buffer "*my-tag*"
	  ...
	  (princ (format "Tag file:  %S (%s)\n" tags-fname
			 (if tags-fname
			     (if (file-exists-p tags-fname) "OK" "missing")
			   "nil")))
