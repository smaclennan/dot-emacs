...
;; $Source: /cvs/netweb/www/people/gfe/ksh-mode.el,v $ --  
;;
;; LCD Archive Entry:
;; ksh-mode|Gary Ellison|gfe@interhack.net
;; |Mode for editing sh/ksh/bash scripts
;; |$Date: 2003/07/23 15:31:53 $|$Revision: 1.2 $|~/modes/ksh-mode.el.Z|
...
;;
;; Delta On   $Date: 2003/07/23 15:31:53 $
...
;;
;; HISTORY 
;; 13-Apr-00            Gary Ellison
;;    Minor syntax table character class changes.
;;
;; 06-Apr-00            Kevin Blake <kblake@directlink.net>
;;    Better fontification of shell variables.
;;
...
;;
(defvar ksh-keywords nil)
;'("for" "in" "do" "done" "select" "case" "esac" "if"
			...
			(if (ksh-search-forward-sexp ksh-keywords-re fence-post)
			    (progn
				  ...
				  (+ (ksh-current-indentation)
				     ksh-case-item-offset)
				;; For single line if we may have to reset
				(let ((keyword (match-string 0)))
				  (if (and (string= keyword "if")
					   (re-search-forward "\\<fi\\>" fence-post t))
				      (progn
					(back-to-indentation)
					(ksh-current-indentation))
				    (if (and (string-match "^\\(while\\|for\\)$" keyword)
					     (re-search-forward "\\<done\\>" fence-post t))
					(progn
					  (back-to-indentation)
					  (ksh-current-indentation))
				      (+ (ksh-current-indentation)
